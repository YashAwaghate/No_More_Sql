...
def epoch_acc(model, batch_size, sql_data, table_data, save_results = False):
    ...
    while start < len(sql_data):
        ...
        q_seq= [['what', 'is', 'terrence', 'ross', "'", 'nationality']]
        ...
        table_id = table_ids[0] #table no.
        table_info = table_data[table_id]
        table = Table(table_id, table_info["header"], table_info["types"], table_info["rows"])
            
        predicted_query_object = Query.from_dict(pred_queries[0])
        print("readable query=",table.query_str(predicted_query_object))

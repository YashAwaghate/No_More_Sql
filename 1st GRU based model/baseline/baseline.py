...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    encoder_optimizer.zero_grad()  #to set gradients to zero before starting to do backpropragation because PyTorch accumulates the gradients
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            decoder_input = target_tensor[di]  # Teacher forcing> using real target outputs as each next input, instead of using the decoder's guess as the next input.
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            decoder_input = topi.squeeze().detach()   #squeeze removes dim of size 1
...
def run_baseline():
    ...
    trainIters(encoder1, decoder1, 10000, print_every=1000, plot_every=1000,learning_rate=lr_best)
    acc = evaluateRandomly(encoder1, decoder1, n=1000) 
